    ...
    if opcode in ("LOG0", "LOG1", "LOG2", "LOG3", "LOG4") and len(stack) > 1:
        ...
        if isinstance(stack[2], six.integer_types):
            ...
            if check_solver(solver) == unsat:
                gas_increment += GCOST["Gcallvalue"]
            solver.pop()
    elif opcode == "SHA3" and isinstance(stack[1], six.integer_types):
        pass # Not handle
...
def update_analysis(analysis, opcode, stack, mem, global_state, path_conditions_and_vars, solver):
    ...
    if opcode == "CALL":
        ...
        recipient = stack[0]
        if not isinstance(recipient, six.integer_types):
            recipient = simplify(recipient)
    ...
    # this is for data flow
        ...
        if opcode == "SLOAD":
            ...
            if len(stack) > 1:
                ...
                if not isinstance(stored_address, six.integer_types):
